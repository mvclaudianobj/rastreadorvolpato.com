...
public function salvar_monitoramento() {
		$url = 'https://seguidor.com.br/pluga.co/automacao_captacao.php';
		$this->enviarCaptacaoGPI( $this->input->post(),$url);
...
*/
	...
	public function salvar() {
		$arrayInput =  $this->input->post();   
		$arrayInput['captacao_telefone1'] = "(".$this->input->post('captacao_ddd').")" . $this->input->post('captacao_telefone1');
		unset($arrayInput['captacao_ddd']);
		$data = $this->formatarDados($arrayInput);
		$this->db->insert('captacao', $data);
		if($data){
			$this->enviarCaptacaoGPI($data);
			$this->enviarCaptacaoSeguidor($data);
		}
...
private function formatarDados($Dados){
    ...
    private function enviarCaptacaoSeguidor($Dados){
		...
		$Dados['captacao_fonte_oportunidade']='5ef38e2d557b0a000e31d6c7';
